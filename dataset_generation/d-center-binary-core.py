...
``(scores_np, boxes_cxcywh_np)`` 列表（与 :mod:`yolo_model.decode_yolo_outputs`
对齐；CenterNet 由 gate.py 用 ``centers → 占位 box`` 的小适配器对齐到同款接口）。
...
#   - YOLO 直接用 yolo_model.decode_yolo_outputs
...
def _draw_detection(vis, cx, cy, w_box, h_box, color=(0, 220, 0)):
    """根据 box 尺寸自适应：``w/h > 0`` 画矩形+实心十字（YOLO 真框）；
...
def process_fits_list(fits_list, save_path, model, class_model, process_config, decode_fn,
    ...
    decode_fn :
        ``decode_fn(raw_pred, conf_thr, iou_thr) -> list[(scores_np, boxes_cxcywh_np)]``。
        YOLO 直接用 :func:`yolo_model.decode_yolo_outputs`；CenterNet 由 gate.py 把
        `decode_centernet_outputs`（返回 centers）适配成同款接口（centers → 占位 box）。
    n_search_files :
    ...
    if candidate_manifest:
        manifest_dir = os.path.dirname(candidate_manifest)
        if manifest_dir:
    ...
    for i in range(0, search_limit, block_file):
        ...
        for j in range(num_slices):
            ...
            yolo_input = img_batch_flat.unsqueeze(1).contiguous()    # (N, 1, 512, 512)
            with torch.no_grad():
                raw_pred = model(yolo_input)
            ...
            for item in positive_candidates:
                ...
                if process_config.save_plot:
                    ...
                    # YOLO 真框 → 矩形+实心十字；CenterNet 仅中心点（w=h=0）→ 空心十字
