...
``(scores_np, boxes_cxcywh_np)`` 列表。当前只保留 CenterNet 检测器，gate.py 会把
``centers`` 适配成 ``cx, cy, w=0, h=0`` 的统一候选格式。
...
class ProcessConfig:
    ...
    class_time_downsample: int   = 1
    class_threshold:       float = 0.5
    classifier_batch_size: int   = 64
    dedup_dm_tolerance:    float = 0.0
    dedup_time_tolerance_ms: float = 0.0
    save_npy:              bool  = True
    save_plot:             bool  = True
    verbose:               bool  = False
...
def _validate_config(process_config, block_size, down_file_leng):
    ...
    """
    errors = []
    dm_stride = _effective_dm_stride(process_config)
    if process_config.dm_range < process_config.dm_span:
        errors.append(f'dm_range {process_config.dm_range} 必须不小于 dm_span '
                      f'{process_config.dm_span}')
    if process_config.dm_span % 512 != 0:
        errors.append(f'dm_span {process_config.dm_span} 必须是 512 的整数倍')
    if dm_stride <= 0:
        errors.append(f'dm_stride {dm_stride} 必须为正数')
    if block_size % 512 != 0:
        errors.append(f'block_size {block_size} 必须是 512 的整数倍')
    class_time_downsample = _classifier_time_downsample(process_config)
    if process_config.class_block_size <= 0:
        errors.append(f'class_block_size {process_config.class_block_size} 必须为正数')
    if class_time_downsample <= 0:
        errors.append(f'class_time_downsample {class_time_downsample} 必须为正数')
    elif process_config.class_block_size % class_time_downsample != 0:
        errors.append(f'class_block_size {process_config.class_block_size} 必须能被 '
                      f'class_time_downsample {class_time_downsample} 整除')
...
def _effective_dm_stride(process_config):
    """Return configured DM chunk stride, preserving old non-overlap behavior by default."""
    dm_stride = int(getattr(process_config, 'dm_stride', 0) or 0)
    return int(process_config.dm_span) if dm_stride <= 0 else dm_stride


def _dm_chunk_starts(process_config):
    """DM-index starts for 512x512 detector images.

    With dm_span=1024 and dm_stride=512, dm_range=2048 produces starts
    [0, 512, 1024], i.e. [0,1024], [512,1536], [1024,2048].
    """
    dm_span = int(process_config.dm_span)
    dm_range = int(process_config.dm_range)
    dm_stride = _effective_dm_stride(process_config)
    last_start = dm_range - dm_span
    starts = list(range(0, last_start + 1, dm_stride))
    if not starts or starts[-1] != last_start:
        starts.append(last_start)
    return starts


def _classifier_time_downsample(process_config):
    """Return classifier time downsample factor, preserving old behavior by default."""
    return int(getattr(process_config, 'class_time_downsample', 1) or 1)


def _maybe_downsample_classifier_time(new_burst, process_config):
    """Average adjacent classifier time rows before normalization when requested."""
    factor = _classifier_time_downsample(process_config)
    if factor <= 1:
        return new_burst
    return new_burst.reshape(
        new_burst.shape[0] // factor,
        factor,
        new_burst.shape[1],
    ).mean(dim=1)


def _deduplicate_positive_candidates(positive_candidates, process_config):
    """Suppress duplicate final positives in physical TOA-DM space.

    Image-space NMS cannot merge overlap-window duplicates because the same
    physical DM has different y pixels in different DM windows.  This keeps the
    highest detector/classifier score candidate within a TOA-DM neighborhood.
    """
    dm_tol = float(getattr(process_config, 'dedup_dm_tolerance', 0.0) or 0.0)
    time_tol_ms = float(getattr(process_config, 'dedup_time_tolerance_ms', 0.0) or 0.0)
    if dm_tol <= 0.0 or time_tol_ms <= 0.0 or len(positive_candidates) <= 1:
        return positive_candidates

    time_tol_samp = time_tol_ms / (time_reso * down_time_rate * 1e3)
    ranked = sorted(
        positive_candidates,
        key=lambda item: (
            float(item['cand']['score']),
            float(item['cls_score']),
        ),
        reverse=True,
    )
    kept = []
    for item in ranked:
        ...
        for kept_item in kept:
            kept_cand = kept_item['cand']
            if (
                abs(float(cand['dm_value']) - float(kept_cand['dm_value'])) <= dm_tol
                and abs(int(cand['toa_samp']) - int(kept_cand['toa_samp'])) <= time_tol_samp
            ):
                duplicate = True
                break
        if not duplicate:
...
#   - CenterNet 由 gate.py 用 (centers → 占位 box) 的适配器对齐到同款接口
...
def _draw_detection(vis, cx, cy, w_box, h_box, color=(0, 220, 0)):
    """画出检测中心；保留矩形参数以兼容旧 manifest 或未来带框检测器。"""
...
def _save_plot(img_display, new_burst, out_path, dm_start, block_size, process_config):
    ...
    plt.yticks(np.linspace(0, 512, 5),
               ((np.linspace(0, process_config.dm_span, 5) + dm_start)
...
def process_fits_list(fits_list, save_path, model, class_model, process_config, decode_fn,
    ...
    decode_fn :
        ``decode_fn(raw_pred, conf_thr, iou_thr) -> list[(scores_np, boxes_cxcywh_np)]``。
        gate.py 把 `decode_centernet_outputs`（返回 centers）适配成统一候选接口。
    n_search_files :
    ...
    proposal_manifest = str(getattr(process_config, 'proposal_manifest', '') or '')
    for manifest_path in (candidate_manifest, proposal_manifest):
        if manifest_path:
    ...
    if process_config.verbose:
        print(f"[core] block_file={block_file} comb_file={comb_file}", flush=True)
    ...
    # 检测图几何：dm_range 沿 DM 轴切成若干张 512x512 检测图。
    # 默认 dm_stride=dm_span 保持旧的无重叠切片；dm_stride<dm_span 时生成重叠图。
    dm_span        = process_config.dm_span                    # 每张检测图覆盖的原始 DM 点数
    dm_stride      = _effective_dm_stride(process_config)
    dm_chunk_starts = _dm_chunk_starts(process_config)
    num_dm_chunks  = len(dm_chunk_starts)                       # DM 轴上切出的检测图数量
    ...
    for i in range(0, search_limit, block_file):
        ...
        if process_config.verbose:
            print(f"[core] processing_block_start={filename}", flush=True)
        ...
        if process_config.verbose:
            print(f'Processing {num_slices} slices, {num_dm_chunks} DM chunks, dm_stride={dm_stride}')
            if class_time_downsample > 1:
                print(f'Classifier time downsample={class_time_downsample}')

        for j in range(num_slices):
            # ── 切片 → 降采样 → 归一化（全在显存）──────────────────────────
            slice_data     = new_data_t[:, :, j * block_size: (j + 1) * block_size]
            # 对每个 DM window 单独 pool；这样支持 [0,1024], [512,1536], [1024,2048]
            # 这类重叠窗口，同时保留默认无重叠行为。
            img_batch      = torch.stack([
                slice_data[:, dm_start: dm_start + dm_span, :]
                .reshape(3, 512, dm_per_pixel, 512, time_pool)
                .mean(dim=(2, 4))
                for dm_start in dm_chunk_starts
            ], dim=1)                                               # (3, num_dm_chunks, 512, 512)
            ...
            detector_input = img_batch_flat.unsqueeze(1).contiguous()    # (N, 1, 512, 512)
            with torch.no_grad():
                raw_pred = model(detector_input)
            ...
            for idx, (pred_scores, pred_boxes) in enumerate(det_results):
                ...
                for box_idx, (cx, cy, w_box, h_box) in enumerate(pred_boxes.astype(np.int64)):
                    ...
                    dm_value  = cy * dm_per_pixel + dm_start
                    ...
                    new_burst = _maybe_downsample_classifier_time(new_burst, process_config)
                    new_burst = preprocess_data_gpu(new_burst)

                    candidates.append({
                        'proposal_uid': (
                            f"{filename}-TS{j:02d}-IMG{idx:02d}-BX{box_idx:03d}"
                            f"-DM{dm_value:.3f}-TOA{toa_samp}"
                        ),
                        'box_idx': box_idx, 'idx': idx,
                        'cx': int(cx), 'cy': int(cy), 'w_box': int(w_box), 'h_box': int(h_box),
                        'dm_value': float(dm_value), 'score': score_val,
                        'dm_start': int(dm_start),
                        'toa_samp': int(toa_samp), 'start_samp': int(start_samp),
            ...
            positive_candidates = [
                {'cand': cand, 'cls_score': float(cls_score)}
                for cand, cls_score in zip(candidates, class_scores)
                if cls_score > process_config.class_threshold
            ]
            positive_candidates = _deduplicate_positive_candidates(positive_candidates, process_config)
            kept_proposal_uids = {item['cand']['proposal_uid'] for item in positive_candidates}

            # proposal_manifest 记录所有通过 CenterNet 阈值和 DM 阈值的 proposal，
            # 包括 classifier 拒绝的候选；candidate_manifest 仍只记录最终正样本。
            if proposal_manifest:
                with open(proposal_manifest, 'a', encoding='utf-8') as handle:
                    for cand, cls_score in zip(candidates, class_scores):
                        k, d = divmod(cand['idx'], num_dm_chunks)
                        cls_score = float(cls_score)
                        record = {
                            'run_label': getattr(process_config, 'run_label', ''),
                            'proposal_uid': cand['proposal_uid'],
                            'candidate_stage': 'detector_proposal',
                            'fits_file': fits_list[i],
                            'fits_stem': filename,
                            'save_path': save_path,
                            'time_slice': int(j),
                            'frequency_split': int(k),
                            'dm_chunk': int(d),
                            'dm_start': int(cand['dm_start']),
                            'candidate_index': int(cand['box_idx']),
                            'image_index': int(cand['idx']),
                            'cx': int(cand['cx']),
                            'cy': int(cand['cy']),
                            'w_box': int(cand['w_box']),
                            'h_box': int(cand['h_box']),
                            'detector_score': float(cand['score']),
                            'centernet_score': float(cand['score']),
                            'classifier_score': cls_score,
                            'detector_threshold': float(process_config.det_prob),
                            'classifier_threshold': float(process_config.class_threshold),
                            'passed_classifier_threshold': bool(cls_score > process_config.class_threshold),
                            'kept_after_dedup': bool(cand['proposal_uid'] in kept_proposal_uids),
                            'pred_dm_pc_cm3': float(cand['dm_value']),
                            'pred_toa_downsampled_sample': int(cand['toa_samp']),
                            'pred_toa_raw_sample': int(cand['toa_samp'] * down_time_rate),
                            'start_raw_sample': int(cand['start_samp'] * down_time_rate),
                            'time_reso_seconds': float(time_reso),
                            ...
                            'class_block_size': int(process_config.class_block_size),
                            'class_time_downsample': int(class_time_downsample),
                            'dm_span': int(process_config.dm_span),
                            'dm_stride': int(dm_stride),
                        }
            ...
            for item in positive_candidates:
                ...
                if candidate_manifest:
                    record = {
                        ...
                        'detector_score': float(cand['score']),
                        'centernet_score': float(cand['score']),
                        'classifier_score': float(cls_score),
                        'detector_threshold': float(process_config.det_prob),
                        'classifier_threshold': float(process_config.class_threshold),
                        'pred_dm_pc_cm3': float(cand['dm_value']),
                        'pred_toa_downsampled_sample': int(cand['toa_samp']),
                    ...
                    with open(candidate_manifest, 'a', encoding='utf-8') as handle:
                        handle.write(json.dumps(record, sort_keys=True) + '\n')
